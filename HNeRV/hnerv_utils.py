...
def loss_fn(pred, target, loss_type='L2', batch_average=True, masked_loss=False, weighted_loss=False, foreground_mask=None):
    target = target.detach()
    if loss_type == 'L2':
        if weighted_loss or masked_loss:
            loss = F.mse_loss(pred, target, reduction='none')
            if weighted_loss:
                # If loss is weighted, then the background loss must be weighted by 0.5
                loss = loss * foreground_mask + loss * ~foreground_mask * 0.5
            elif masked_loss:
                loss = loss * foreground_mask
