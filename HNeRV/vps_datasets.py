...
class CityscapesVPSVideoDataSet(Dataset):
    def __init__(self, args):
        ...
        self.panoptic_categories, self.panoptic_images, self.panoptic_annotations = self.load_json(args.anno_path)
        self.rgb_img_name_suffix = "_leftImg8bit.png"

        rgb_mask_names = [file for file in sorted(os.listdir(args.panoptic_video_mask_dir)) if file.startswith(vidname)]
        
        self.video = [os.path.join(args.data_path, "_".join(x.split("_")[2:5]) + '_leftImg8bit.png') for x in rgb_mask_names] # e.g. frankfurt_000000_001751      
        
        self.image_ids = ["_".join(mask_name.split("_")[:5]) for mask_name in rgb_mask_names] # e.g. 0005_0025_frankfurt_000000_001751
        self.vid_inst_mask_paths = sorted(glob.glob(args.panoptic_inst_mask_dir + "/" + vidname + "*.png"))
        self.vid_rgb_mask_paths = sorted(glob.glob(args.panoptic_video_mask_dir + "/" + vidname + "*.png"))
    ...
    def load_annos(self, inst_mask_path, rgb_mask_path, image_id):
        ...
        panoptic_annos = [ann for ann in self.panoptic_annotations if ann['image_id'] == image_id][0]
...
class VIPSegVideoDataSet(Dataset):
    def __init__(self, args):
        ...
        self.panoptic_categories, self.panoptic_videos, self.panoptic_annotations = self.load_json(args.anno_path)        
        ...
        self.video = [os.path.join(args.data_path, vidname, x + '.jpg') for x in self.image_ids]

        self.vid_inst_mask_paths = [os.path.join(args.panomasks_dir, vidname, x + '.png') for x in self.image_ids]
        self.vid_rgb_mask_paths = [os.path.join(args.panomasksRGB_dir, vidname, x + '.png') for x in self.image_ids]
    ...
    def img_load(self, idx):
        # img = np.array(Image.open(self.video[idx]))
        img = read_image(self.video[idx])
        return img / 255.
    ...
    def load_annos(self, inst_mask_path, rgb_mask_path, image_id):
        ...
        for el in np.unique(panop_mask):
            ...
            # For the thing class, we can get the category by // 100, otherwise it's already the category id
            if el < 125:
                semantic_id = el
                is_crowd = 0
            else:
...
class VideoFlowDataSet(Dataset):
    def __init__(self, args):
        ...
        self.video = [os.path.join(args.image_dir, file) for file in sorted(os.listdir(args.image_dir))]
        self.flow_images = [os.path.join(args.flow_dir, file) for file in sorted(os.listdir(args.flow_dir))] # npy files
    ...
    def load_flow(self, idx):
        ...
        # Discard the small negative values
    ...
    def img_transform(self, img, flow_img=None):
        ...
        if self.resize_list != '-1':
            if '_' in self.resize_list:
                ...
                img = interpolate(input=img.unsqueeze(0), size=(resize_h, resize_w), mode='bicubic').squeeze(0)
                if flow_img is not None:
                    flow_img = interpolate(input=flow_img.unsqueeze(0), size=(resize_h, resize_w), mode='bicubic').squeeze(0)
            else:
                resize_hw = int(self.resize_list)
                img = resize(img, resize_hw,  'bicubic')
                if flow_img is not None:
    ...
    def __getitem__(self, idx):
        ...
        sample = {'img': tensor_image, 'idx': idx, 'norm_idx': norm_idx, 'flow_img': tensor_flow_img}
