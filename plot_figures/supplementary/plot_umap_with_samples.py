...
def save_umap(dump_path, layer_name, data, data_ids, n_neighbors=15, min_dist=0.1, n_components=2, 
    ...
    # Set x, y ticks and labels off for plot
    plt.tick_params(axis='both', left=False, top=False, right=False, bottom=False, labelleft=False, labeltop=False, labelright=False, labelbottom=False)
    
    plt.savefig(os.path.join(dump_path, f'{layer_name}_umap.png'), bbox_inches='tight', dpi=500, pad_inches=0)
...
def main():
    ...
    # Do one (dataset, video) pair at a time to be compatible with the code
    for (dataset_name, vid_name) in dataset_vid_name_pairs:
        print(dataset_name, vid_name)
        
        dump_path = f'../outputs/supplementary/heatmaps/{dataset_name}/{vid_name}/'
        ...
        for i in range(num_samples):
            ...
            plt.imshow(rgb_img)
            plt.axis('off')
            plt.savefig(os.path.join(dump_path, f'{i}.png'), bbox_inches='tight', dpi=500, pad_inches=0)
        ...
        for inr_idx, inr_type in enumerate(inrs_to_consider):
            ...
            for layer_idx, layer_name_to_take in enumerate(layer_names[inr_type]):
                row_num_in_plot = 1 + inr_idx * (5 - len(layer_names[inr_type])) + layer_idx            
                ...
                # For this plot, each video has one seed only
                # So use one seed for the uMAP
                ...
                cluster_point_info, cluster_id_data = save_umap(
                    dump_path, '_'.join(layer_name_to_take.split('_')[:2]), data, data_ids, n_neighbors=neighbours,  n_components=2, metric='euclidean', title='UMAP projection of the HNeRV vectors', kmeans=True, num_clusters=num_clusters, axis_off=True
                )
                
                # Select a few (4) points from 4 clusters to plot
                
                num_clusters_to_plot = 4
                num_heatmaps_per_cluster = 4
                for cluster_id in range(num_clusters_to_plot):
                    all_ids = np.arange(len(data_ids))
                    for class_id in [0]:
                        ids = (cluster_id_data==cluster_id) & (data_ids==class_id)
                        ids_to_consider = all_ids[ids]
                        # select 4 random indices of all_kernels[ids_to_consider]
                        chosen_heatmaps = all_kernels[ids_to_consider][np.random.choice(len(ids_to_consider), num_heatmaps_per_cluster)]
                
                    # Create a white strip with nans
                    white_strip_v = np.ones((chosen_heatmaps[0].shape[0], 1)) * np.nan
                    white_strip_h = np.ones((1, chosen_heatmaps[0].shape[1])) * np.nan

                    # Create a 2x2 grid with white strips
                    heatmap_grid = np.vstack([
                        np.hstack([chosen_heatmaps[0], white_strip_v, chosen_heatmaps[1]]),
                        np.hstack([white_strip_h, np.ones((1, 1)) * np.nan, white_strip_h]),
                        np.hstack([chosen_heatmaps[2], white_strip_v, chosen_heatmaps[3]])
                    ])
                    ...
                    plt.savefig(os.path.join(dump_path, f'{layer_name}_c{cluster_id}.png'), bbox_inches='tight', dpi=500, pad_inches=0)
