...
class single_image_cityscape_vps_dataset(Dataset):
    def __init__(self, cfg):
        ...
        rgb_mask_names = [
            file
            for file in sorted(os.listdir(cfg.data.panoptic_video_mask_dir))
        ...
        img_path = os.path.join(
            cfg.data.data_path,
            "_".join(rgb_mask_name.split("_")[2:5]) + "_leftImg8bit.png",
        )

        image_id = "_".join(
            rgb_mask_name.split("_")[:5]
        )  # e.g. 0005_0025_frankfurt_000000_001751
        print("image_id: ", image_id)
        inst_mask_path = sorted(
            glob.glob(cfg.data.panoptic_inst_mask_dir + "/" + vidname + "*.png")
        )[0]
        rgb_mask_path = sorted(
            glob.glob(cfg.data.panoptic_video_mask_dir + "/" + vidname + "*.png")
        )[0]
    ...
    def load_annos(
        self, anno_path, inst_mask_path, rgb_mask_path, image_id, resize=None
    ):
        panoptic_categories, panoptic_images, panoptic_annotations = self.load_json(
            anno_path
        )

        # Load the panoptic annotation from JSON for this image_id
        panoptic_annos = [
            ann for ann in panoptic_annotations if ann["image_id"] == image_id
        ][0]
...
class single_image_vipseg_dataset(Dataset):
    def __init__(self, cfg):
        ...
        self.panoptic_categories, self.panoptic_videos, self.panoptic_annotations = (
            self.load_json(cfg.data.anno_path)
        )
        ...
        image_id = sorted([img_info["id"] for img_info in vid_info["images"]])[
            frame_num_in_video
        ]  # e.g. 00000166
        ...
        inst_mask_path = os.path.join(
            cfg.data.panomasks_dir, vidname, image_id + ".png"
        )
        rgb_mask_path = os.path.join(
            cfg.data.panomasksRGB_dir, vidname, image_id + ".png"
        )
    ...
    def load_annos(
        self, inst_mask_path, rgb_mask_path, image_id, resize=None, crop=None
    ):
