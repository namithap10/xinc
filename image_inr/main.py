...
def get_trainer(cfg,logger,callbacks):
    ...
    trainer = L.Trainer(accelerator='gpu',strategy=cfg.trainer.strategy,\
                        max_epochs=cfg.trainer.num_iters,\
                            default_root_dir=cfg.logging.checkpoint.logdir,\
                                fast_dev_run = cfg.trainer.debug,logger=logger,\
                            enable_checkpointing= (not cfg.logging.checkpoint.skip_save_model),\
                                callbacks=callbacks,\
                                    check_val_every_n_epoch=cfg.trainer.eval_every,devices=cfg.trainer.devices)
    return trainer
    ...
    def forward(self,batch,kwargs,reshape=False):
        ...
        if reshape:
            if self.cfg.data.patch_shape is not None:
                ...
                pred = einops.rearrange(pred, '(h w) c ph pw -> c (h ph) (w pw)', h=num_patches_h, w=num_patches_w)
...
def main(cfg: DictConfig):
    ...
    if cfg.logging.wandb.enabled:
        ...
        #just passign resume=true should not crash.
        ...
        logger = WandbLogger(project=wandb_opt.project,group=wandb_opt.group,save_dir=cfg.logging.wandb.dir,\
                             entity=wandb_opt.entity,config=wandb_config,resume=resume,id=wandb_id)
        cfg.logging.wandb.id = wandb_id
                
    else:
        logger = TensorBoardLogger(save_dir=cfg.logging.checkpoint.logdir,name='tensorboard_logs')
