...
class ComputeMLPContributions:
    ...
    def compute_all_layer_mappings(self):
        ...
        layer_1_input, layer_2_input, layer_3_input = (
            self.intermediate_results[-4],
            self.intermediate_results[-3],
            self.intermediate_results[-2],
        )

        in_feats = [
            layer_1_weight.shape[1],
            layer_2_weight.shape[1],
            layer_3_weight.shape[1],
        ]
        out_feats = layer_3_weight.shape[0]

        layer_1_output_contrib = torch.zeros(in_feats[0], H * W)
        layer_1_output_contrib_to_layer2infeat = torch.zeros(
            in_feats[0], in_feats[1], H * W
        )

        layer_2_output_contrib = torch.zeros(in_feats[1], H * W)
        layer_2_output_contrib_to_layer3infeat = torch.zeros(
            in_feats[1], in_feats[2], H * W
        )

        layer_3_output_contrib = torch.zeros(in_feats[2], H * W)  # e.g. 128 x hw
        layer_3_output_contrib_to_rgb = torch.zeros(in_feats[2], out_feats, H * W)

        with torch.no_grad():
            # Layer 3
            for in_feat in range(layer_3_output_contrib.shape[0]):
                contrib = torch.mm(
                    layer_3_input[:, in_feat].unsqueeze(1),
                    layer_3_weight[:, in_feat].unsqueeze(1).T,
                )  # hw x 3
                # Add bias
            ...
            for in_feat in range(layer_2_output_contrib.shape[0]):
                contrib = torch.mm(
                    layer_2_input[:, in_feat].unsqueeze(1),
                    layer_2_weight[:, in_feat].unsqueeze(1).T,
                )
                # Add bias
            ...
            for in_feat in range(layer_1_output_contrib.shape[0]):
                contrib = torch.mm(
                    layer_1_input[:, in_feat].unsqueeze(1),
                    layer_1_weight[:, in_feat].unsqueeze(1).T,
                )
                # Add bias
        ...
        # Reshape contribution maps
        layer_1_output_contrib = einops.rearrange(
            layer_1_output_contrib, "f (h w) -> f h w", h=H, w=W
        )
        layer_2_output_contrib = einops.rearrange(
            layer_2_output_contrib, "f (h w) -> f h w", h=H, w=W
        )
        layer_3_output_contrib = einops.rearrange(
            layer_3_output_contrib, "f (h w) -> f h w", h=H, w=W
        )
        layer_3_output_contrib_to_rgb = einops.rearrange(
            layer_3_output_contrib_to_rgb, "f c (h w) -> f c h w", h=H, w=W
        )
        ...
        return (
            ...
            layer_1_output_contrib_to_layer2infeat,
            layer_2_output_contrib_to_layer3infeat,
            layer_3_output_contrib_to_rgb,
        )
