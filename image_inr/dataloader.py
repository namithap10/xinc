...
class single_image_dataset(Dataset):
    def __init__(self,cfg):
        ...
        self.features,self.features_shape = self.proc.get_features(self.data.unsqueeze(0),patch_shape=cfg.data.patch_shape)
    ...
    def load_tensor(self,path,resize=None, crop=None):
        ...
        if resize is None:
            ...
            custom_transforms = tv.transforms.Compose([tv.transforms.Resize(resize),tv.transforms.ToTensor()])
...
class single_image_dataset_with_annos(Dataset):
    def __init__(self,cfg):
        ...
        self.proc = data_process.DataProcessor(cfg.data)
        self.data = self.load_tensor(path,cfg.data.data_shape,crop=cfg.data.crop)
        self.annotations = self.load_annotations(anno_path,image_id,cfg.data.data_shape,crop=cfg.data.crop)
        #Converts data to [N,3]
        # Features shape can be different from data shape as we might pad. 
...
class INRDataset(Dataset):
    def __init__(self,cfg,data_path):
        ...
        self.ckpt_files = glob.glob(data_path+'/**/*.ckpt',recursive=True)
        self.ckpt_files = sorted(self.ckpt_files)

        model_config = data_path +'/exp_config.yaml'
        if os.path.exists(model_config):
            self.model_config = model_config #omegaconf.OmegaConf.load(model_config)
    ...
    def __getitem__(self, index):
        ckpt_file = self.ckpt_files[index]
        ckpt = torch.load(ckpt_file,map_location='cpu')
        state = ckpt['state_dict']
        pos_matrix,mlp_matrix = helper.get_mlp_matrix(state,self.model_config)

        batch = {}
        if not self.cfg.data.ignore_pos_matrix:
            batch['pos_matrix'] = pos_matrix
        batch['mlp_matrix'] = mlp_matrix
        batch['ckpt_path'] = ckpt_file
